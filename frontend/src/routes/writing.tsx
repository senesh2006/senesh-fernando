...
export const Route = createFileRoute("/writing")({
  ...
  head: ({ loaderData }) => {
    const posts = loaderData?.posts ?? FALLBACK_POSTS;
    return {
    meta: [
      { title: "Writing — Senesh Fernando" },
      { name: "description", content: "Field notes on software, design systems, performance, and shipping fast." },
      { property: "og:title", content: "Writing — Senesh Fernando" },
      { property: "og:description", content: "Field notes on software, design systems, performance, and shipping fast." },
      { property: "og:image", content: posts[0]?.cover },
    ],
  };
  },
...
function WritingLanding() {
  useReveal();
  const { posts: POSTS } = Route.useLoaderData();

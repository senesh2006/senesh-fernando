...
import type { FlipCardData } from "@/components/flip-card";
import { submitContact } from "@/lib/api";
import { useState } from "react";

const CONTACT_EMAIL =
  import.meta.env.VITE_CONTACT_EMAIL || "seneshfernando55@gmail.com";

const GITHUB_USER =
  import.meta.env.VITE_GITHUB_USERNAME || "senesh2006";

export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      { title: "Contact — Senesh Fernando" },
      { name: "description", content: "Get in touch with Senesh Fernando for engineering and design work." },
      { property: "og:title", content: "Contact — Senesh Fernando" },
      { property: "og:description", content: "Get in touch for engineering and design work." },
    ],
  }),
      ...
      <section className="relative border-b border-border">
        ...
        <div className="mx-auto max-w-6xl px-5 sm:px-8 py-20 sm:py-32 relative">
          ...
          <div className="mt-10 flex flex-wrap gap-2 font-mono text-xs">
            ...
            <Link to="/projects" className="px-3 py-1.5 border border-border rounded-sm link-hover">→ see the work first</Link>
          </div>
        </div>
        <MarqueeStrip items={[

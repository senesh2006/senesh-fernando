...
export const Route = createFileRoute("/projects/$slug")({
  ...
  notFoundComponent: () => (
    <div className="mx-auto max-w-3xl px-5 sm:px-8 py-32 text-center">
      <div className="font-mono text-xs text-muted-foreground mb-4">// 404.project</div>
      <h1 className="text-4xl font-semibold tracking-tight">Project not found.</h1>
      <Link to="/projects" className="inline-block mt-6 font-mono text-sm link-hover px-2 py-1 rounded-sm border border-border">← back to projects</Link>
    </div>
  ),
...
function ProjectPage() {
  useReveal();
  const { project: p, projects: PROJECTS } = Route.useLoaderData() as {
    project: Project;
    projects: Project[];
  };
    ...
    <>
      ...
      <article className="relative">
        ...
        <nav className="mx-auto max-w-6xl px-5 sm:px-8 mt-24 mb-24 border-t border-border pt-10 reveal">
          ...
          <Link to="/projects/$slug" params={{ slug: next.slug }} className="group flex items-baseline justify-between gap-6">

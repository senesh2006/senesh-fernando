...
import type { BackendBlog, BackendExperience, BackendProject } from "@/lib/backend-types"
import {
  estimateReadMinutes,
  extractBlogHtml,
  looksLikeHtml,
...
export function mapSkillNames(records: { name: string }[]) {
  return records.map((record) => record.name).filter(Boolean)
}

...
// Sample recommendations - in a real app these would come from a database
const initialRecommendations: Recommendation[] = [
  {
    id: 1,
    name: "Dr. Sarah Chen",
    role: "Senior Lecturer",
    company: "Curtin University Colombo",
    message: "Senesh consistently demonstrates exceptional analytical skills and a genuine passion for problem-solving. His work on the Critical Care Optimization System showed remarkable understanding of algorithm design and real-world application.",
    date: "2024",
  },
  {
    id: 2,
    ...
    message: "Working with Senesh on data visualization projects was a great experience. He brings creative solutions to complex problems and has excellent communication skills.",
    date: "2024",
  },
]
          ...
          {recommendations.map((rec, index) => (
            ...
            >
              ...
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-[rgba(255,106,0,0.2)] flex items-center justify-center">
                  <span className="text-[#ff6a00] font-semibold text-lg">
                    {rec.name.split(' ').map(n => n[0]).join('')}
                  </span>
                </div>
                <div>
                  <h3 className="font-semibold text-[#f5ede6]">{rec.name}</h3>
                  <p className="text-sm text-[rgba(245,237,230,0.5)]">
                    {rec.role} at {rec.company}
                  </p>
                </div>
                <span className="ml-auto text-xs text-[rgba(245,237,230,0.4)]">{rec.date}</span>
              </div>
            </div>
          ))}
        </div>
